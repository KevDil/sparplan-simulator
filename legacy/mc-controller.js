    ...
    for (const workerItem of workers) {
      workerItem.worker.onmessage = (e) => {
        ...
        if (type === 'chunk-progress') {
          ...
          if (progressCallback) {
            const currentCompleted = completedChunks.reduce((sum, c) => sum + c.rawData.finalTotals.length, 0);
            progressCallback({
              current: currentCompleted + completedInChunk,
              total: iterations,
              percent: Math.round(((currentCompleted + completedInChunk) / iterations) * 100),
            });
          }
